...
async function runLoop(
	...
	while (true) {
		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			stream.push({ type: "turn_end", message, toolResults });
			telemetry.log("turn", telemetry.end("turn"));
...
async function executeToolCall(
	...
	signal: AbortSignal | undefined,
	stream: EventStream<AgentEvent, AgentMessage[]>,
	options: { emitMessageEvents: boolean },
	...
	try {
		...
		const handled = errorHandler.handle(e, { phase: "tool_execution", toolName: toolCall.name });
		result = {
...
async function executeToolCalls(
	...
	const shouldParallelize = toolCalls.length > 1 && toolCalls.every((toolCall) => toolCall.name === "task");

	if (shouldParallelize) {
		const toolResults = await Promise.all(
			toolCalls.map((toolCall) => executeToolCall(toolCall, tools, signal, stream, { emitMessageEvents: false })),
		);
