...
import type { HookRunner } from "./utils/hook-runner.js";
import {
	DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES,
	formatSize,
...
const DEFAULT_BLOCKED_PATTERNS = [/\brm\s+-rf\s+\/$/, /:\(\)\s*\{\s*:\|:\s*&\s*\};:/];
...
export interface BashToolOptions {
	...
	/** Hook runner for shell.env hook injection */
	hookRunner?: HookRunner;
	/** Command validation options */
	security?: BashSecurityConfig;
	/** Track recent commands for debugging/auditing */
	commandHistory?: string[];
}

export function createBashTool(cwd: string, options?: BashToolOptions): AgentTool<typeof bashSchema> {
	const ops = options?.operations ?? defaultBashOperations;
	const builder = new BashCommandBuilder(options?.commandPrefix);
	const hookRunner = options?.hookRunner;
	const securityPatterns = options?.security?.blockedPatterns ?? DEFAULT_BLOCKED_PATTERNS;
	...
	return {
		...
		execute: async (
			...
			const resolvedCommand = builder.build(command);
			...
			const hookEnv = hookRunner?.hasHandlers("shell.env")
				...
				: undefined;
			const safeEnv: NodeJS.ProcessEnv | undefined = hookEnv
				? (Object.fromEntries(
						Object.entries(hookEnv).filter(([, value]) => typeof value === "string"),
			...
			return new Promise((resolve, reject) => {
				...
				ops.exec(resolvedCommand, cwd, { onData: handleData, signal, timeout, env: safeEnv })
					.then(({ exitCode }) => {
