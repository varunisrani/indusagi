...
import { SimpleOptionsProviderError as ProviderError, type ProviderLifecycleHooks, executeWithRetry, normalizeProviderError } from "./simple-options.js";
...
export async function executeOpenAIRequest<TRequest, TResponse>(
	request: TRequest,
	executor: () => Promise<TResponse>,
	options?: OpenAIRequestExecutionOptions<TRequest, TResponse>,
): Promise<TResponse> {
	await options?.hooks?.onBeforeRequest?.(request);
	try {
		const response = await executeWithRetry(executor, {
			maxAttempts: options?.retryPolicy?.maxAttempts ?? 3,
			baseDelayMs: options?.retryPolicy?.baseDelayMs ?? 250,
		});
		await options?.hooks?.onAfterResponse?.(response);
		return response;
	} catch (error) {
...
export async function processResponsesStream<TApi extends Api>(
	...
	for await (const event of openaiStream) {
		if (event.type === "response.output_item.added") {
			...
			throw new ProviderError(`Error Code ${event.code}: ${event.message}` || "Unknown error", "unknown", event);
		} else if (event.type === "response.failed") {
			throw new ProviderError("Unknown error", "unknown", event);
		}
	}
}
