...
/**
 * Google Vertex AI stream handler.
 *
 * Handles streaming responses from Google Vertex AI API and emits
 * standardized assistant events.
 */
class GoogleVertexStreamHandler extends BaseStreamHandler {
...
export const streamGoogleVertex: StreamFunction<"google-vertex", GoogleVertexOptions> = (
	...
	const handler = new GoogleVertexStreamHandler(async () => {
		try {
			...
			const googleStream = await executeGoogleRequest(
				params,
...
export const streamSimpleGoogleVertex: StreamFunction<"google-vertex", SimpleStreamOptions> = (
	...
	const effort = mapThinkingLevel(options.reasoning, "clamp-xhigh") as ClampedThinkingLevel;
