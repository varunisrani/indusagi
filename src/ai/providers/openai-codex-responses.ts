...
import { buildBaseOptions, mapThinkingLevel, normalizeProviderError } from "./simple-options.js";
...
/**
 * OpenAI Codex Responses stream handler.
 *
 * Handles streaming responses from OpenAI Codex's Responses API and emits
 * standardized assistant events.
 */
class OpenAICodexResponsesStreamHandler extends BaseStreamHandler {
...
export const streamOpenAICodexResponses: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	const handler = new OpenAICodexResponsesStreamHandler(async () => {
		try {
			...
			stream.pushDone(output.stopReason as "stop" | "length" | "toolUse", output);
			stream.end();
		} catch (error) {
			const normalized = normalizeProviderError(error);
			output.stopReason = options?.signal?.aborted ? "aborted" : "error";
			output.errorMessage = normalized.message;
