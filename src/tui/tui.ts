...
export abstract class ComponentBase implements Component {
	...
	protected renderCached(width: number, renderFn: () => string[]): string[] {
...
export class FocusManager {
	...
	setFocus(component: Component | null): { previous: Component | null; current: Component | null } {
		const previous = this.focused;
		if (isFocusable(previous)) previous.focused = false;
		previous?.onBlur?.();
...
export class RenderEngine {
	diffLines(oldLines: string[], newLines: string[]): { firstChanged: number; lastChanged: number; appended: boolean } {
		let firstChanged = -1;
		let lastChanged = -1;
		...
		if (appended) {
			if (firstChanged === -1) firstChanged = oldLines.length;
...
export class LayoutEngine {
	resolveOverlayLayout(
		...
		} else row = this.resolveAnchorRow(opt.anchor ?? "center", effectiveHeight, availHeight, marginTop);
		...
		} else col = this.resolveAnchorCol(opt.anchor ?? "center", width, availWidth, marginLeft);
...
export class Container extends ComponentBase {
	...
	clear(): void {
		for (const child of this.children) child.onUnmount?.();
		this.children = [];
	}
...
export class TUI extends Container {
	...
	private prepareRenderFrame(width: number, height: number): { newLines: string[]; cursorPos: { row: number; col: number } | null } {
	...
	private computeRenderDiff(newLines: string[]): {
		firstChanged: number;
		lastChanged: number;
